...
export default function MemberDetails({ user = {}, member = {} }) {
    // department-moderator.approve-member
    const handleApproveJoinRequest = () => {
        Swal.fire({
            title: "Are you sure?",
            text: `Do you want to approve ${member.name}'s request?`,
            icon: "warning",
            ...
            if (result.isConfirmed) {
                // send a get request to the named route with name  department-moderator.approve-member and pass the member id as parameter
                router.get(
                    route("department-moderator.approve-member", {
                        member: member.id,
                    }),
                );
            }
    ...
    // department-moderator.remove-approval

    const handleRemoveApproval = () => {
        Swal.fire({
            title: "Are you sure?",
            text: `Do you want to remove approval for ${member.name}?`,
            icon: "warning",
            showCancelButton: true,
            confirmButtonColor: "#000",
            cancelButtonColor: "slategray",
            ...
            if (result.isConfirmed) {
                // send a get request to the named route with name  department-moderator.remove-approval and pass the member id as parameter
                router.get(
                    route("department-moderator.remove-approval", {
                        member: member.id,
                    }),
                );
            }
